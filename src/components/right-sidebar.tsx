...
export function RightSidebar() {
  ...
  const festivalsQuery = query(collection(db, 'festivals'), where('status', '==', 'published'), orderBy('date', 'desc'), limit(1));
  const [upcomingFestivals, festivalsLoading] = useCollectionData(festivalsQuery, { idField: 'id' });

  const deitiesQuery = query(collection(db, 'deities'), where('status', '==', 'published'), orderBy('name_en', 'asc'), limit(1));
  ...
  const storiesQuery = query(collection(db, 'stories'), where('status', '==', 'published'), orderBy('createdAt', 'desc'), limit(1));
  const [featuredSagas, sagasLoading] = useCollectionData(storiesQuery, { idField: 'id' });

  const templesQuery = query(collection(db, 'temples'), where('status', '==', 'published'), orderBy('name_en', 'asc'), limit(1));
              ...
              <Link href={`/contests`} key={contest.id} className="block group">
                <div className="flex items-center gap-3">
                  <div className="relative w-12 h-12 rounded-md overflow-hidden shrink-0 bg-secondary">
                    {imageUrl && <SafeImage src={imageUrl} alt={contest.title} data-ai-hint={contest.imageHint} fill className="object-cover" sizes="48px" />}
                  </div>
                  <div>
          ...
          {sagasLoading ? <Loader2 className="mx-auto animate-spin" /> : featuredSagas?.map((saga: DocumentData) => {
            ...
            return (
              <Link href={`/stories/${saga.slug}`} key={saga.id} className="block group">
                <div className="flex items-center gap-3">
                  <div className="relative w-12 h-12 rounded-md overflow-hidden shrink-0 bg-secondary">
                      {imageUrl && <SafeImage src={imageUrl} alt={title} data-ai-hint={saga.image.hint} fill className="object-cover" sizes="48px" />}
                  </div>
                  <div>
          ...
          {templesLoading ? <Loader2 className="mx-auto animate-spin" /> : popularTemples?.map((temple: DocumentData) => {
            ...
            return (
              <Link href={`/temples/${temple.slug}`} key={temple.id} className="block group">
                <div className="flex items-center gap-3">
                  <div className="relative w-12 h-12 rounded-md overflow-hidden shrink-0 bg-secondary">
                      {imageUrl && <SafeImage src={imageUrl} alt={title} data-ai-hint={temple.media.images[0].hint} fill className="object-cover" sizes="48px" />}
                  </div>
                  <div>
                      <p className="font-semibold text-sm group-hover:text-primary line-clamp-2">{title}</p>
          ...
          {festivalsLoading ? <Loader2 className="mx-auto animate-spin" /> : upcomingFestivals?.map((festival: DocumentData) => {
            ...
            return (
              <Link href={`/festivals/${festival.slug}`} key={festival.id} className="block group">
                <div className="flex items-center gap-3">
                  <div className="relative w-12 h-12 rounded-md overflow-hidden shrink-0 bg-secondary">
                    {imageUrl && <SafeImage src={imageUrl} alt={name} fill className="object-cover" sizes="48px" />}
                  </div>
                  <div>
